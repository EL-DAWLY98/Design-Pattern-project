...
A: Yes! Edit the color constants at the top of `CarWashGUI.java` (`BG_DARK`, `ACCENT_CYAN`, etc.)

**Q: How do I add a new wash type?**
A: Create a new class implementing `WashService`, then update `WashFactory.createWash()`

**Q: How do I export transaction data?**
A: The system automatically generates `CarWash_Transaction_History.html` which can be opened in any browser

**Q: Can this run on Mac/Linux?**
A: Yes! Adapt the `run.bat` to a shell script with the same Java command
